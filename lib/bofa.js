...
exports.BofAClient = void 0;
const axios_1 = __importDefault(require("axios"));
const fingerprints_json_1 = __importDefault(require("./data/fingerprints.json"));
const logger_1 = require("./logger");
const moment_1 = __importDefault(require("moment"));
const tos_json_1 = __importDefault(require("./data/tos.json"));
const filterRules_json_1 = __importDefault(require("./data/filterRules.json"));
const dashboard_json_1 = __importDefault(require("./data/dashboard.json"));
...
const DEFAULT_HEADERS = {
    ...
    channel: "MBDA",
    "BE-APP-ORIGIN": "MDA",
    "device-id": "ipAddress=&fsoToken=&browserString=Mozilla/5.0 (Linux; U; Android 12; en-US) MDA/22.09.04&cookieToken=",
...
const http = (0, axios_cookiejar_support_1.wrapper)(axios_1.default);
class BofAClient {
    constructor(options = {}) {
        ...
        this.jar =
            (options.cookie && tough_cookie_1.CookieJar.deserializeSync(options.cookie)) ||
    ...
    async collectAuthHeaders(response) {
        ...
        const encryptedGUID = authResponse?.onlineToken?.tokenSets?.[1]?.value?.cipherData?.value;
        ...
        const ssoKey = response?.data?.stepUpToken?.processContext?.ssoEncryptionKey;
        if (encryptedGUID) {
            this.encryptedGUID = encryptedGUID;
        }
        if (tracer)
            this.traceToken = tracer;
        if (ssoKey)
            this.ssoKey = ssoKey;
        let ecidentifier;
        for (var i = 0; i < 2; i++) {
        ...
        if (scp?.[0] && scp?.[1] && this.securedContactPoint) {
            if (scp[0].deliveryMethod == "TEXT") {
                ...
                this.securedContactPoint["securedContactPoint"].maskedContactPoint.description = "";
            }
            else {
    ...
    async _call(path, method, data = null, extraHeaders = {}) {
        try {
            ...
            if (typeof response.data === "string" &&
                response.data.match("This page is used to hold your data while you are being authorized for your request.")) {
    ...
    async firstUse() {
        return await this._call("https://mservice.bankofamerica.com/mgateway/public/v1/appInitialize/BA_ANDROID_HANDSET/22.09.04/firstUse", "get");
    }
    async initMobileAuth() {
        const response = await this._call("https://mservice.bankofamerica.com/mgateway/public/initiateMobileAuthentication/v1", "post", tos_json_1.default, {
            channelLOB: "SMRTPH",
            devicePlatform: "Google",
        });
        return response;
    }
    async zelleHistory(o) {
        const { fromDate, toDate } = o || {};
        const response = await this._call("https://mservice.bankofamerica.com/mgateway/transfer-funds-ws/api/transaction/v3/history", "post", {
            filter: {
                ...
                toDate: toDate || (0, moment_1.default)(new Date()).format("YYYY-MM-DD"),
            },
            sortCriteria: {
                fieldName: "Date",
                order: "ASCENDING",
    ...
    async transferDetails() {
        const response = await this._call("https://mservice.bankofamerica.com/mgateway/csb/transfers-ws/rest/v7/transfer-details.json", "post", {
            acctIdentifier: {},
            opCode: "",
        });
        return response;
    ...
    async getAlias() {
        const response = await this._call("https://mservice.bankofamerica.com/mgateway/csb/transfers-ws/omni/mm/rest/v3/alias/get", "post", {
            opCode: "AT_GA",
        });
        return response.data;
    }
    async request({ opCode, alias, memotext, name, requestor, amount }) {
        const response = await this._call("https://mservice.bankofamerica.com/mgateway/csb/transfers-ws/rest/v5/money-requests/request-money.json", "post", {
            moneyRequest: {
                memotext,
                requestor: requestor && {
                ...
                responder: alias && [
                    {
                        ...
                        responderInfo: alias && {
                            aliasToken: alias,
                            aliasTokenType: (alias.match(/^\d+$/) && "PHONE") ||
                                (alias.match("@") && "EMAIL") ||
                                "NAME",
                            name: name && {
                                displayName: name,
    ...
    async accountsOverview() {
        return (await this._call("https://mservice.bankofamerica.com/mgateway/myaccounts-ws/rest/v6/accountsoverview.filter", "post", {})).data;
    }
    async sendOtp() {
        const response = await this._call("https://mservice.bankofamerica.com/mgateway/authws/rest/sas/ls/secure/v1/sendCode", "post", Object.assign(this.securedContactPoint, filterRules_json_1.default));
        return response.data;
    }
    async validateOtp({ otp }) {
        const authCode = { authenticationCode: otp };
        const response = await this._call("https://mservice.bankofamerica.com/mgateway/public/v18/signon/OTP/validate", "post", Object.assign(authCode, filterRules_json_1.default));
        return response.data;
    }
    async zelleInit() {
    ...
    async getCards() {
        return (await this._call("https://mservice.bankofamerica.com/mgateway/csb/administer-accounts-ws/omni/card/v8/settings", "post", {
            metadata: {},
            payload: {
    ...
    async accountNumber() {
        return (await this._call("https://mservice.bankofamerica.com/mgateway/csb/myaccounts-ws/rest/details/v3a/attribute.filter?key=identifier", "post", {
            secure: {
    ...
    async recipients({ numInZelleBucket1, opCode }) {
        return (await this._call("https://mservice.bankofamerica.com/mgateway/csb/transfers-ws/rest/v4/recipients.json", "post", {
            numInZelgleBucket1: numInZelleBucket1 || 6,
            opCode: opCode || "AT_GR",
        })).data;
    }
    async transfer({ opCode, duplicateFlag, amount, displayName, adx, payeeId, nickName, payeeAliasToken, }) {
        if (!opCode)
            opCode = "AT_SMT";
        if (!["AT_VMT", "AT_SMT"].includes(opCode))
            throw Error("op-code invalid"); // AT_SMT to transfer
        if (nickName)
            payeeId = (await this.recipients({})).recipients.find((v) => v.nickName === nickName).identifier;
        return (await this._call("https://mservice.bankofamerica.com/mgateway/csb/transfers-ws/rest/v7/money-transfers.json", "post", {
            displayName,
            opCode,
            duplicateFlag,
            transaction: {
    ...
    async start() {
        return (await this._call("https://mservice.bankofamerica.com/mgateway/public/v1/appInitialize/BA_ANDROID_HANDSET/22.09.04/subsequentUse", "get")).data;
    }
    async resumeSession() {
        await this.start();
    ...
    async registerDevice() {
        ...
        const response = await this._call("https://mservice.bankofamerica.com/mgateway/authws/rest/sas/ls/v1/recordDevicePreference", "post", Object.assign(auth, filterRules_json_1.default));
        return response.data;
    }
    async dashboard() {
        return (await this._call("https://mservice.bankofamerica.com/mgateway/v3/mobileDashboard ", "post", dashboard_json_1.default)).data;
    }
    async txHistory({ accountToken, subAccountToken }) {
        const { data } = await this._call("https://mservice.bankofamerica.com/mgateway/csb/myaccounts-ws/rest/details/v4/activity.filter", "post", { secure: { accountToken: accountToken || this.tokenId, subAccountToken } });
    ...
    async cardholders({ accountToken }) {
        return (await this._call("https://mservice.bankofamerica.com/mgateway/myaccounts-ws/rest/details/v3a/cardholders.filter", "post", {
            metadata: {},
            secure: {
                accountToken,
            },
    ...
    async next({ nextItemToken, accountToken, subAccountToken }) {
        const { data } = await this._call("https://mservice.bankofamerica.com/mgateway/csb/myaccounts-ws/rest/details/v4/transactions.filter", "post", {
            metadata: {
                paging: {
    ...
    async retrieveSignInHistory() {
        return (await this._call("https://mservice.bankofamerica.com/mgateway/authws/sc/sm/incoming/v9/rerieveSignInHistory", "post", { RetrieveSignInHistoryRequest: {} })).data;
    }
    async eligibleArrangements({ accountToken, featureName, filter }) {
        return await this._call("https://mservice.bankofamerica.com/mgateway/csb/customer-ws/rest/v2/eligible-arrangements.json", "post", {
            accountToken: accountToken || this.tokenId,
            featureName,
            filter,
        });
    ...
    async activate({ adx, cvv, expirationMonth, expirationYear }) {
        ...
        return await this._call("post", "https://mservice.bankofamerica.com/mgateway/csb/v1/card/activate", {
    ...
    async createOneTimeToken({ softToken }) {
        return await this._call("https://mservice.bankofamerica.com/mgateway/csb/authws/common/sm/incoming/customerSoftToken/createOneTimeToken", {
            CreateSoftTokenRequest: {
                softToken,
            },
        });
    }
    async addRecipient({ accountNumber, adx, firstName, lastName, }) {
    ...
    async signIn({ username, password }) {
        ...
        const uids = username;
        const pwd = password;
        const uencryptedData = crypto_1.default.publicEncrypt({
            key: publicKey,
            padding: crypto_1.default.constants.RSA_PKCS1_OAEP_PADDING,
        }, 
        // We convert the data string to a buffer using `Buffer.from`
        Buffer.from(uids));
        const pencryptedData = crypto_1.default.publicEncrypt({
            key: publicKey,
            padding: crypto_1.default.constants.RSA_PKCS1_OAEP_PADDING,
        }, 
        // We convert the data string to a buffer using `Buffer.from`
        ...
        const response = await this._call("https://mservice.bankofamerica.com/mgateway/public/v18/signon", "post", data, {
            channelLOB: "SMRTPH",
...
exports.BofAClient = BofAClient;
//# sourceMappingURL=bofa.js.map